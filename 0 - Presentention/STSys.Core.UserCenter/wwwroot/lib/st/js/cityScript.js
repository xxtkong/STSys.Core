...
citySelector.pc[31] = new Array("港澳台", "香港|澳门|台湾");

citySelector.hotCity = ["北京", "贵州", "上海", "广州", "深圳"];
...
citySelector.template = '<div class="city-box CityBox" id="js_cityBox"><div class="prov-city" id="js_provCity"><p id="">热门城市</p><ul>' + citySelector.hotCityhtmls + '</ul></div><div class="provence"><div id="">选择省份</div><div><ul id="js_provList">' + citySelector.provHtmls + '</ul></div></div></div>';
...
citySelector.cityInit = function (input) {
    $("#" + input + ",." + input + "").click(function (event) {
        ...
        var _input = input;
        $("#js_cityBox").on("click", ".js_cityName", function (event) {
            var event = event || window.event;
            if (event && event.stopPropagation) {
                event.stopPropagation();
            } else {
        ...
        // 点击页面其他地方
        var $js_cityBox = $('#js_cityBox');
        $('html').on('click', function (event) {
            // console.log('点击了html');
            var targetId = event.target ? event.target.id : event.srcElement.id;
            // console.log(targetId);
            if (targetId != 'js_provCity') {
                // console.log('二者不等');
                $js_cityBox.remove();
                return;
            }else {

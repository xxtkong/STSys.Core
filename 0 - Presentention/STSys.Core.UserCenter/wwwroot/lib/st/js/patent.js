...
$(function () {
    ...
    $('.patentBtn').on('click', function () {
        var $ContentName1 = $('.patent_formbox [name="ContentName1"]').val();
        $('.formSubmit2 [name="ContentName2"]').val($ContentName1);
        $('.pop_div,.patentPop').show();
    });
    //提交信息成功的弹窗里的确认
    $(".confirm_btn").click(function () {
        $(".pop_success_wrap,.pop_div").hide();
        $('.patent_formbox [name="ContentName1"]').val('');
    });
    // 查询专利按钮
    $('.queryPatentBtn').on('click', function () {
        $('.pop_div,.patentPop').show();
    });
    // 查询专利弹窗中的获取结果
    ...
    function submitForm() {
        if ($(".formSubmit2 [name='ContentName2']").val() == "") {
            layer.msg("请输入您的专利名称", { time: 3000, icon: 5 });
            return;
        }
        ...
        //var model = $(".patentPop .formSubmit2").serialize();
